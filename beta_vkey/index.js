...
if(!localStorage.getItem('fh_list_keyv')){
  localStorage.setItem('fh_list_keyv',JSON.stringify([]));
}
var playlist=JSON.parse(localStorage.getItem('fh_list_keyv'));
...
playlistul.addEventListener('click',function(e){  
  function clli(l){
    ...
    if(l.tagName=='LI'){
      ...
      kugou.getSongDetails(id,function(res){
        if(res.error){
          alert('歌曲获取失败！');
          removeSong(nowplay);
          return;
...
function addSong(song){
  ...
  localStorage.setItem('fh_list_keyv',JSON.stringify(playlist));
}
